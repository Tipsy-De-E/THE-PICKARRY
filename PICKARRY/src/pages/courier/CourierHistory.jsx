...
import {
  ...
  CheckCircle, PlayCircle, TruckIcon, AlertTriangle, Download, Loader2,
...
const CourierHistory = () => {
  ...
  const checkAuthentication = async () => {
    try {
      ...
      await fetchCourierOrders(courierData.id);
      setupRealtimeSubscription(courierData.id);
  ...
  // FIXED: Load customer feedback for all orders
  const loadCustomerFeedback = async (orderIds) => {
    try {
      console.log('Loading customer feedback for order IDs:', orderIds);

      if (!orderIds || orderIds.length === 0) {
        console.log('No order IDs provided for customer feedback');
        setCustomerFeedback({});
        return;
      }
  ...
  // Fetch courier orders from Supabase - ONLY CURRENT DELIVERIES (not booked)
  const fetchCourierOrders = async (courierId) => {
    try {
      console.log('Fetching current delivery orders for courier:', courierId);
      ...
      const { data: ordersData, error } = await supabase
        ...
        .eq('book_for_delivery', false) // ONLY current deliveries (not booked)
        .order('created_at', { ascending: false });

      if (error) throw error;

      console.log('Fetched current delivery orders:', ordersData);

      // Format orders for display
      const formattedOrders = ordersData.map(order => {
        const timeline = order.order_status_history?.map(history => ({
          status: history.courier_status || history.status,
        ...
        return {
          ...
          date: `Today: ${order.delivery_time || new Date(order.created_at).toLocaleTimeString()}`,
          status: order.status === 'pending' ? 'Pending' :
            order.status === 'accepted' ? 'In Progress' :
              order.status === 'delivered' ? 'Completed' :
                order.status === 'cancelled' ? 'Cancelled' : order.status,
          deliveryStatus: order.courier_status || 'pending',
          ...
          earnings: order.status === 'delivered' ? '50.00' : '0.00',
          timeline: timeline,
          book_for_delivery: order.book_for_delivery,
          supabaseData: order,
          fare_breakdown: fareBreakdown,
          is_rush_delivery: order.is_rush_delivery,
          rush_amount: order.rush_amount || 0,
      ...
      // FIXED: Load customer feedback AFTER orders are set
      const orderIds = formattedOrders.map(order => order.id);
      await loadCustomerFeedback(orderIds);

      // FIXED: Load remarks AFTER orders are set
      await loadRemarks(courierId);
  ...
  // FIXED: Load remarks for the courier (FROM courier TO customer)
  const loadRemarks = async (courierId) => {
    try {
      console.log('Loading remarks for courier:', courierId);
      ...
      const remarksObj = {};
      data.forEach(item => {
        remarksObj[item.order_id] = item.remark;
      });

      console.log('Processed remarks:', remarksObj);
  ...
  const setupRealtimeSubscription = (courierId) => {
    const subscription = supabase
      .channel(`courier-current-orders-${courierId}`)
      .on('postgres_changes',
        {
          event: '*',
          schema: 'public',
        ...
        (payload) => {
          ...
          fetchCourierOrders(courierId); // Refresh orders
        }
      )
      .on('postgres_changes',
        {
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          ...
          loadRemarks(courierId);
          const orderIds = orders.map(order => order.id);
          if (orderIds.length > 0) {
            loadCustomerFeedback(orderIds);
          }
        }
      )
      .subscribe();
  ...
  const updateDeliveryStatus = async (orderId, newStatus) => {
    try {
      ...
      const statusMap = {
        'picked_up': { status: 'accepted', courier_status: 'picked_up' },
        'on_the_way': { status: 'accepted', courier_status: 'on_the_way' },
        'arrived': { status: 'accepted', courier_status: 'arrived' },
        'delivered': { status: 'delivered', courier_status: 'delivered' }
      };

      const statusUpdate = statusMap[newStatus];
      if (!statusUpdate) {
      ...
      await supabase
        ...
        .insert({
          ...
          courier_status: statusUpdate.courier_status,
        ...
        if (order) {
          await supabase
            ...
            .insert({
              ...
              order_id: orderId,
              amount: 50.00, // Standard earnings for delivery
              type: 'delivery',
      ...
      // ENHANCED: Send appropriate notifications based on status
      try {
        const order = orders.find(o => o.id === orderId);
        if (order && order.supabaseData) {
          if (newStatus === 'picked_up' || newStatus === 'on_the_way' || newStatus === 'arrived') {
            // Use delivery progress notifications for intermediate statuses
            ...
            // Use order status update for final delivery
  ...
  // Enhanced cancel order function with notifications
  const cancelOrder = async (orderId) => {
    try {
      ...
      // ENHANCED: Send cancellation notification to customer
      try {
        const order = orders.find(o => o.id === orderId);
        if (order && order.supabaseData) {
          await notificationService.notifyOrderStatusUpdate(
  ...
  // ENHANCED: Enhanced remarks modal for feedback (matching CustomerOrders structure)
  ...
  // ENHANCED: Save remark with feedback notification (matching CustomerOrders structure)
  const saveRemark = async () => {
    try {
      if (selectedOrderId && currentOrderRemark.trim()) {
        ...
        // ENHANCED: Send courier feedback notification to customer
        try {
  ...
  // Get next available status for the order
  const getNextStatusOptions = (order) => {
    ...
    return statusFlow[order.deliveryStatus] || [];
  };
...
Generated on: ${new Date().toLocaleString()}
  ...
  return (
    <div className="courier-home">
            ...
            {filteredOrders.length === 0 ? (
              activeTab === 'current' ? (
                <div className="no-orders-container">
                  ...
                  <h3 className="no-orders-title">No Active Deliveries</h3>
                  <p className="no-orders-text">
                    There are currently no active deliveries. Accepted orders will appear here.
                  </p>
                </div>
              ) : (
                <div className="no-orders-container">
                  <Package size={48} className="no-orders-icon" />
                  <h3 className="no-orders-title">No Delivery History</h3>
                  ...
                  <div className="order-details">
                    ...
                    <div className="order-detail">
                      ...
                      <span className="detail-right detail-right-payment">
                        {order.paymentMethod.includes('GCash') ? 'GCash' : 'COD'}: ₱{order.totalAmount.toFixed(2)}
                      </span>
                    </div>
                    <div className="order-detail">
                    ...
                    {activeTab === 'current' && (
                      <>
                        {order.status === 'Pending' ? (
                          ...
                          // For Accepted and In Progress orders - show Update Status button only
        ...
        <div className="modal-overlay" onClick={() => setShowOrderDetailsModal(false)}>
          <div className="order-details-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-bodys order-details-body">
              ...
              {/* Enhanced Action Buttons for Current Orders */}
              {activeTab === 'current' && (
              ...
              {/* Action Buttons for History Orders */}
              {activeTab === 'history' && (
                <div className="modal-actions">
                  <button
                    className="cancel-button secondary"
            ...
            <div className="modal-bodys">
              ...
              <div className="status-update-section">
                ...
                <div className="notification-note">
                  <Bell size={16} />
                  <span>Customer will receive real-time notifications about delivery progress.</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ENHANCED REMARKS MODAL - Matching CustomerOrders Structure */}
